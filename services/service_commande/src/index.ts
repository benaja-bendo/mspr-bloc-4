...
async function init() {
  if (process.env.DATABASE_URL && Pool) {
    ...
    await pool.query('CREATE TABLE IF NOT EXISTS orders (id SERIAL PRIMARY KEY, clientId INT, productId INT)');
  }
  ...
  if (process.env.KAFKA_BROKER) {
    ...
    if (kafka) {
      ...
      await consumer.run({ eachMessage: async ({ message }) => console.log(`orders service received: ${message.value}`) });
    }
  }
}
...
app.post('/orders', async (req: Request, res: Response) => {
  ...
  let order: Order;
  if (pool) {
    const result = await pool.query('INSERT INTO orders(clientId, productId) VALUES($1,$2) RETURNING id, clientId, productId', [clientId, productId]);
    order = result.rows[0];
  } else {
  ...
  if (producer) {
    await producer.send({ topic: 'orders-topic', messages: [{ value: JSON.stringify({ type: 'order-created', payload: order }) }] });
  }
